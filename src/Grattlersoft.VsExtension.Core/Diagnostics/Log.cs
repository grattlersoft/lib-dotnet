...
/// <summary>
/// Zentraler Logger fuer alle Softfair VS Extensions.
...
public static class Log
{
    ...
    private static Guid GenerateGuidFromName(string name)
    {
        ...
        var hash = md5.ComputeHash(System.Text.Encoding.UTF8.GetBytes("Softfair.VsExtension." + name));
...
#pragma warning disable VSTHRD010 // OutputStringThreadSafe ist per Design thread-safe
    private static void Write(string level, string message)
    {
        ...
        lock (_lock)
        {
            ...
            if (_targets.Count == 0)
                WriteToFile(Path.Combine(Path.GetTempPath(), "Softfair.VsExtension.log"), line);
        }
    }
